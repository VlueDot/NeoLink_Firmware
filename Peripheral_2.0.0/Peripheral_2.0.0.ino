...
String get_gps (unsigned long timeout){

unsigned long gps_init_time = millis();
char ch;
char _recipe[100];


while (millis()-gps_init_time<timeout*1000)
{
  ...
  if(gps.available()) {
    ...
    //Serial.write(ch);
    memset(_recipe, '\0', 100);
    if (ch=='$'){

      //return "GP$-8.08626$-79.12098$0";

      gps.readBytes(_recipe, 5);
      String temp =String(_recipe);
      //Serial.print(temp);
 
      
      if(temp.equals("GNRMC")) {
          //Serial.print("\n"+temp+": ");
          memset(_recipe, '\0', 100);
          ...
          while (1)
          {
            if(gps.available()){
              ...
              _recipe[i]=gps.read();
              if (_recipe[i]==13) break;
              i=i+1;
              
            }
           
          }
          //Serial.println(_recipe);
          String result = lat_long(_recipe);
          //Serial.println(result);
          //Serial.println();
          if(!result.equals("NaN")) return result;
