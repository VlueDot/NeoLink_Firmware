...
void send_cloud();
void config_LoRa();
void sendMessage(String outgoing,int destination);
...
void setup()
{
  ...
  else if (prg_iteration == 9)
  {
    ...
    if (SN[0] == DEVICE_HEADER[0] && SN[1] == DEVICE_HEADER[1])
      INCOMPATIBILIDAD_FIRMWARE_ERROR = 0;
    else
      INCOMPATIBILIDAD_FIRMWARE_ERROR = 1;
    ...
    if (checking_battery())
      check_registered(); //checking if neolink is registered;
    ...
    if (INCOMPATIBILIDAD_FIRMWARE_ERROR)
      compatibility_error();
...
void get_LOCAL_SN()
{
  ...
  if (!wifi_conf_isSet)
    wifi_conf_isSet = setting_wifi();
  if (WiFi.status() != WL_CONNECTED)
    starting_wifi();
  if (WiFi.status() == WL_CONNECTED)
  {
    unsigned long init_SN_time = millis();
    while (!Firebase.getString(firebasedata, "SN_CHIPS/" + chipid_str + "/SN_LOCAL"))
      if (millis() - init_SN_time > 10000)
        deepsleep_beep(1, 1000);
...
void check_registered()
{
  ...
  if (Start_or_Restart)
  {
    ...
    String real_month;
    if (i_mon < 9)
      real_month = "0" + String(i_mon + 1);
    else
      real_month = String(i_mon + 1);

    String real_day;
    if (i_day < 10)
      real_day = "0" + String(i_day);
    else
      real_day = String(i_day);

    String real_hour;
    if (i_hour < 10)
      real_hour = "0" + String(i_hour);
    else
      real_hour = String(i_hour);

    String real_min;
    if (i_min < 10)
      real_min = "0" + String(i_min);
    else
      real_min = String(i_min);
    ...
    if (cond1 && cond2)
    {
      ...
      if (actual_hour < 10)
        real_hour = "0" + String(actual_hour);
      else
        real_hour = String(actual_hour);

      if (actual_min < 10)
        real_min = "0" + String(actual_min);
      else
        real_min = String(actual_min);

      String real_sec;
      if (actual_secs < 10)
        real_sec = "0" + String(actual_secs);
      else
        real_sec = String(actual_secs);
...
void get_environment_sensor()
{
  ...
  */
  ...
  {
    LOCAL_UPDATE = 0;
    if (Firebase.getInt(firebasedata, DEVICE + PATH_CONFIGURATION_STATUS + "LOCAL_UPDATE"))
      LOCAL_UPDATE = firebasedata.intData();
    ...
    if (LOCAL_UPDATE)
    {
      ...
      if (!FirmwareCheck())
        Firebase.setInt(firebasedata, DEVICE + PATH_CONFIGURATION_STATUS + "LOCAL_UPDATE", 1);
    }
    else
    {
  ...
  while (1)
  {
    ...
    else
    {
      ...
      if (dry_bulb_temp_mov >= 200 && dry_bulb_temp > 0)
        dry_bulb_temp_mov = dry_bulb_temp;
      ...
      {
        if (dry_bulb_temp_mov < 0)
          dry_bulb_temp_mov = dry_bulb_temp;
        else
          dry_bulb_temp_mov = (dry_bulb_temp_mov + dry_bulb_temp) / 2;
      }
...
void get_neolink_status()
{
  ...
  if (internal_temperature == 0)
    internal_temperature = internal_temperature_raw;
...
void send_cloud()
{
  ...
  if (Port1_Active)
  {
    ...
    json_port_status.FirebaseJson::set("Port1_Active", "NaN");
  ...
  if (Port2_Active)
  {
    ...
    json_port_status.FirebaseJson::set("Port2_Active", "NaN");
  ...
  if (Port3_Active)
  {
    ...
    json_port_status.FirebaseJson::set("Port3_Active", "NaN");
  ...
  if (Port4_Active)
  {
    ...
    json_port_status.FirebaseJson::set("Port4_Active", "NaN");

  //----------------------------------------------------END PORT------------------------------------------------------------

  float temp_factor = 0.11;
  if (internal_temperature <= 21.5)
    temp_factor = 0;
  ...
  */
  ...
  String real_secs;
  if (i_secs < 10)
    real_secs = "0" + String(i_secs);
  else
    real_secs = String(i_secs);
  ...
  if (SENSOR_P1)
    p1_depth_err = DEPTH1 == 0;
  if (SENSOR_P2)
    p2_depth_err = DEPTH2 == 0;
  if (SENSOR_P3)
    p3_depth_err = DEPTH3 == 0;
  if (SENSOR_P4)
    p4_depth_err = DEPTH4 == 0;
  ...
  String _month;
  if (i_mon < 9)
    _month = "0" + String(i_mon + 1);
  else
    _month = String(i_mon + 1);
  ...
  */
  ...
  if (Port1_Active)
    json_port1.FirebaseJson::set("/P1/Depth", DEPTH1);
  if (Port2_Active)
    json_port2.FirebaseJson::set("/P2/Depth", DEPTH2);
  if (Port3_Active)
    json_port3.FirebaseJson::set("/P3/Depth", DEPTH3);
  if (Port4_Active)
    json_port4.FirebaseJson::set("/P4/Depth", DEPTH4);
  ...
  if (PORT_RQ)
  {
    ...
    if (Port1_Active)
    {
      Firebase.updateNodeSilent(firebasedata, DEVICE + PATH_DATA + port1_type + timestamp_string, json_port1);
    ...
    if (Port2_Active)
    {
      Firebase.updateNodeSilent(firebasedata, DEVICE + PATH_DATA + port2_type + timestamp_string, json_port2);
    ...
    if (Port3_Active)
    {
      Firebase.updateNodeSilent(firebasedata, DEVICE + PATH_DATA + port3_type + timestamp_string, json_port3);
    ...
    if (Port4_Active)
    {
      Firebase.updateNodeSilent(firebasedata, DEVICE + PATH_DATA + port4_type + timestamp_string, json_port4);
  ...
  if (GPS_CLOUD)
    Firebase.updateNodeSilent(firebasedata, DEVICE + PATH_CONFIGURATION_STATUS + "GPS", json_gps);
  ...
  if (!MODE_PRG)
  {
    ...
    */
    ...
    Firebase.setString(firebasedata, DEVICE + PATH_CONFIGURATION_STATUS + "IP", String(WiFi.localIP().toString()));
...
int16_t real_sleep_time()
{
  ...
  if (SLEEP_TIME_modem < 1)
    SLEEP_TIME_modem = 10; //+10
...
void NodeExistance()
{
  ...
  if (WiFi.status() == WL_CONNECTED)
  {
    ...
    if (node_existance1)
      Serial.println("node_existance1 true.");
    else
    {
      Serial.println("node_existance1 error.");
    }
    if (node_existance2)
      Serial.println("node_existance2 true.");
    else
    {
      Serial.println("node_existance2 error.");
    }
    if (node_existance3)
      Serial.println("node_existance3 true.");
    else
    {
    ...
    if (!node_existance1 && !node_existance2 && !node_existance3)
    {
      ...
      if (Firebase.updateNode(firebasedata, DEVICE, json_default_nodes))
        Serial.println("Creating Config Nodes Succeded..");
      else
      {
...
void read_OpenWeather()
{
  if (WiFi.status() == WL_CONNECTED)
  {
    ...
    Serial.println("WiFi Disconnected. No OpenWeather API requested.");
}
  ...
  if (httpCode > 0)
  {
    ...
    if (httpCode == HTTP_CODE_OK)
    {
      ...
      else
      {
        ...
        if (verify_json(new_firmware_ver, FIRMWARE_VER, built->valueint, chk->valueint))
          update_firmware(file->valuestring);
        else
          Serial.println("No firmware upgrade is necessary");
      }
    }
  }
  else
...
bool verify_json(String new_firmware_ver, String current_firmware_ver, int built, int chk)
{
  ...
  if (built == new_built && chk == new_chk)
  {
    ...
    if (current_major < new_major || current_minor < new_minor || current_patch < new_patch)
    {
      ...
      Serial.println("Lower or equal Firmware found (" + new_firmware_ver + ")");
  }
  else
  {
...
String onReceive(int packetSize)
{
  ...
  if (packetSize == 0)
    return incoming; // if there's no packet, return
