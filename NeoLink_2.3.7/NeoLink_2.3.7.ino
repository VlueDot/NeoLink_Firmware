...
const String FIRMWARE_VER = "2.3.7";
...
#define FIRMWARE_MODE 'PRO'
  ...
  #define FIREBASE_AUTH "P2aDr6F6P1XZQ3zc7k4ABuPBT9o5szLwFHphsqZt"
  #define UPDATE_JSON_URL  "https://test-firmware-neolink.s3.us-east-2.amazonaws.com/firmware_dev.json"
  const String WIFI_SSID_DEFAULT = "Rruiz 2.4GHz";//Rruiz 2.4GHz
  const String WIFI_PSSWD_DEFAULT = "1a23456789abc";//1a23456789abc
  byte localAddress = 0xBB; //address of this device.
  byte destination = 0x01; //destination to send to.
...
void setup() {
  ...
  if (prg_iteration == 0) {
    ...
    if(Update_LocalSN_Flag || char(EEPROM.read(0)) != 'N') get_LOCAL_SN();
    else{
      SN = ""
      for(int i=0; i<11;i++) {
